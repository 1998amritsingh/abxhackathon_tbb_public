...
def file_detect_ocr(file):
    receipt = process_image(file)
    raw_details = extract_raw_details(receipt)
    price_components = filter_price_components(raw_details)

    print(price_components)
...
def extract_raw_details(receipt):
...
def filter_price_components(raw_details):
    ...
    for row in raw_details.split("\n"):
        # check to see if the price regular expression matches the current
        # row
        if re.search(priceRegex, row) is not None:
...
# def main():
#     file = "assets/receipt-dragon-bing-4.jpeg"
#     file_detect_ocr(file)
